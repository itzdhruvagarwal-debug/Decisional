...
export default function BlogPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setStatus({
        type: "success",
        message: "Thank you! We will notify you when guides go live.",
      });
      setEmail("");
    } catch (err: unknown) {
      setStatus({
        type: "error",
        message: (err instanceof Error ? (err instanceof Error ? err.message : String(err)) : String(err)) || "Failed to subscribe. Please try again.",
      });
    } finally {
      setLoading(false);
    }
          ...
          <div className="container" style={{ position: "relative", zIndex: 1, maxWidth: "640px", textAlign: "center" }}>
            ...
            >
              <h3 style={{ fontSize: "16px", fontWeight: 700, marginBottom: "8px" }}>
                Want to get notified when we publish?
              </h3>
              <p style={{ color: "var(--color-text-muted)", fontSize: "14px", marginBottom: "16px" }}>
                Enter your email address below to receive our first playbook.
              </p>
