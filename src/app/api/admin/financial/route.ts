...
async function _handler_GET(req: NextRequest) {
  ...
  if (fmt === "csv") {
    ...
    rows.push({ "Metric": "DEAL STATISTICS", "Value (₹)": "", "Description": "" });
    rows.push({ "Metric": "Total Deals", "Value (₹)": String(financials.deals.total), "Description": "All deals created" });
    rows.push({ "Metric": "Completed Deals", "Value (₹)": String(financials.deals.completed), "Description": "Successfully completed" });
    rows.push({ "Metric": "Active Deals", "Value (₹)": String(financials.deals.active), "Description": "Currently in progress" });
    rows.push({ "Metric": "Disputed Deals", "Value (₹)": String(financials.deals.disputed), "Description": "Under dispute resolution" });
    rows.push({ "Metric": "Cancelled Deals", "Value (₹)": String(financials.deals.cancelled), "Description": "Cancelled deals" });
    rows.push({ "Metric": "Completion Rate", "Value (₹)": `${financials.deals.completionRate}%`, "Description": "Percentage of completed deals" });

    // Add monthly revenue trend
    rows.push({ "Metric": "", "Value (₹)": "─────", "Description": "─────" });
    rows.push({ "Metric": "MONTHLY REVENUE TREND", "Value (₹)": "", "Description": "" });
