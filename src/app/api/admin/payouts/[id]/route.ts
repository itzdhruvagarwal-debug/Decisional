...
export const PUT = apiWrapper(async (req, { params }) => {
  ...
  try {
    if (action === "REJECT") {
      const result = await handleRejectAction(withdrawalId, failureReason!, admin.id);
      return ApiResponse.success(result, "Payout rejected successfully");
    } else {
      const result = await handleApproveAction(withdrawalId, admin.id);
      ...
      return ApiResponse.success(result, "Payout approved successfully");
    }
  } catch (error: unknown) {
