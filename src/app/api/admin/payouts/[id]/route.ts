...
export const PUT = apiWrapper(async (req, { params }) => {
  ...
  try {
    ...
    if (msg === "WITHDRAWAL_NOT_FOUND") {
      return ApiResponse.notFound("Withdrawal not found");
    }
    if (msg === "WITHDRAWAL_ALREADY_PROCESSED") {
      return ApiResponse.conflict("Withdrawal has already been processed");
    }

    return ApiResponse.error("Failed to process payout. Please try again.", 500);
  }
