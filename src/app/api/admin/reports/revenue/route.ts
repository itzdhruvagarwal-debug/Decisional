...
async function _handler(req: NextRequest) {
  ...
  if (fmt === "csv") {
    ...
    // Add summary rows
    rows.push({ "Month": "", "Revenue (₹)": "─────", "GMV (₹)": "─────", "Deals": "─────", "Avg Deal Size (₹)": "─────" });
    rows.push({ "Month": "TOTAL", "Revenue (₹)": paiseToRupees(totalRevenue), "GMV (₹)": paiseToRupees(totalGMV), "Deals": String(totalDeals), "Avg Deal Size (₹)": paiseToRupees(avgDealSize) });
