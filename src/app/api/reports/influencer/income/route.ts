...
async function _handler(req: NextRequest) {
  ...
  if (fmt === "csv") {
    ...
    rows.push(
      ...
      { "Deal ID": "TOTAL", "Brand": `FY ${fy}`, "Campaign": `${deals.length} deals`, "Category": "", "Completed Date": "", "Gross (₹)": paiseToRupees(totalGross), "Platform Fee (₹)": paiseToRupees(totalPlatformFee), "TDS 194-O (₹)": paiseToRupees(totalTDS), "Net Received (₹)": paiseToRupees(totalNet) }
    );
