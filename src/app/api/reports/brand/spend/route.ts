...
async function _handler(req: NextRequest) {
  ...
  if (fmt === "csv") {
    ...
    rows.push(
      ...
      { "Date": "TOTAL", "Campaign": `FY ${fy}`, "Influencer": `${deals.length} deals`, "Platform": "", "Followers": "", "Paid (₹)": paiseToRupees(totalPaid), "Fee (₹)": paiseToRupees(totalPlatformFee), "GST 18% (₹)": paiseToRupees(totalGST), "Invoice Total (₹)": paiseToRupees(totalInvoice) }
    );
