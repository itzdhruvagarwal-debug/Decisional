  ...
  try {
    ...
    if (city) {
      influencerWhere.influencerProfile = {
        ...(influencerWhere.influencerProfile as object),
    ...
    const influencerOrderBy: Record<string, string> = { xp: "desc" };
    ...
    if (filter === "weekly") {
      ...
      const weeklyInfluencers = weeklyTopInfluencers
        .map((d: any) => {
          const profile = profiles.find((p: any) => p.id === d.influencerId);
        ...
        .filter((influencer: any) => influencer && influencer.trustScore >= 51);
    ...
    const hallOfFame = topInfluencers.slice(0, 3).map((u: any, i: number) => ({
      rank: i + 1,
      id: u.id,
    ...
    return NextResponse.json({
      filter: "all-time",
      influencers: topInfluencers.map((u: any) => ({
        id: u.id,
        name: u.influencerProfile?.displayName,
        avatar: u.influencerProfile?.avatar,
...
async function getTopBrands(limit: number, filter: string, city: string) {
  if (filter === "weekly") {
    ...
    return weeklyBrands
      .map((d: any) => {
        const profile = profiles.find((p: any) => p.id === d.brandId);
        if (!profile) return null;
        return {
          id: profile.user.id,
      ...
      .filter((brand: any) => brand && brand.trustScore >= 51);
  }
