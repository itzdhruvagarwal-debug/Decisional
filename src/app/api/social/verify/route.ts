...
async function verifyYouTubeChannel(userId: string, handle: string, userProfile: any) {
...
async function verifyInstagramChannel(userId: string, handle: string, userProfile: any) {
...
async function _handler_POST(request: NextRequest) {
  try {
    ...
    if (session.user.userType !== "INFLUENCER") {
      throw AppError.forbidden("Only influencer accounts can verify social media profiles");
    }
