...
async function verifyTaxCompliance(userId: string) {
  ...
  if (!taxCompliance?.panLast4) {
    throw new Error("PAN tax compliance is required before withdrawals");
  }
}
  ...
  try {
    ...
    try {
      ...
      return ApiResponse.error(
        "Idempotency-Key header is required and must be 16-128 characters (alphanumeric, -, _, :)"
      );
    }
    ...
    let payoutDetails;
    try {
      payoutDetails = await getPayoutDetails(session.user.id, parsed.data);
    } catch (err: any) {
