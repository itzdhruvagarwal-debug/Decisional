...
export default function DisputeDetailPage({
    ...
    fetch(`/api/disputes/${id}`)
      ...
      .then((data) => {
        if (data.dispute) {
          ...
          } else if (data.dispute.influencerOutcome || data.dispute.brandOutcome) {
            try {
      ...
      .catch((err) => {
        if (err?.name !== "AbortError") {
          console.error("[dispute-detail] Failed to load dispute:", err);
        }
      })
  ...
  const handleAddEvidence = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error("[dispute-detail] Failed to add evidence:", error);
  ...
  const handleDisputeAction = async (
    ...
    try {
      ...
      console.error("[dispute-detail] Failed to perform dispute action:", error);
      showToast("error", "Something went wrong");
    } finally {
          ...
          <div>
            ...
            <div className="card">
              ...
              {dispute.evidence.length === 0 ? (
                ...
                >
                  {dispute.evidence.map((ev: { id: string; type: string; url?: string; description?: string; submittedAt?: string; submittedByUserId?: string }) => (
                    <div
                      key={ev.id}
                      style={{
                        padding: "12px",
