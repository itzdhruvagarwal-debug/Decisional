...
export default function DealDetailPage() {
  ...
  const getFlatDeliverablesList = (dealObj: DealDetail | null | undefined) => {
    ...
    deliverables.forEach((d: { type: string; count: number }) => {
      const typeLabel = d.type.replace(/_/g, ' ').toLowerCase().replace(/\b\w/g, (c: string) => c.toUpperCase());
  ...
  const handleAction = async (action: string, payload: Record<string, unknown>) => {
    ...
    try {
      ...
      showToast("error", (err instanceof Error ? (err instanceof Error ? err.message : String(err)) : String(err)));
      return false;
    } finally {
  ...
  const handleReviewContent = async () => {
    ...
    reviewsPayload.forEach((r) => {
      if (r.status === "REVISION_REQUESTED" && (!r.feedback || r.feedback.length < 5)) {
        showToast("error", `Please provide at least 5 characters of feedback for ${r.type.replace(/_\d+$/, '').replace(/_/g, ' ')}`);
                              ...
                              <div key={item.type} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "8px", background: "var(--color-bg-secondary)", borderRadius: "var(--radius-sm)" }}>
                                <div style={{ display: "flex", flexDirection: "column", gap: "2px" }}>
                                  <span style={{ fontSize: "13px", fontWeight: 600 }}>
                                    {item.type.replace(/_\d+$/, '').replace(/_/g, ' ').toLowerCase().replace(/\b\w/g, (c: string) => c.toUpperCase())}
                                  </span>
                                ...
                                <span style={{
                                  ...
                                  padding: "2px 6px",
                                  borderRadius: "var(--radius-sm)",
                                  background: item.status === "APPROVED" ? "var(--color-success)" : item.status === "REVISION_REQUESTED" ? "var(--color-warning)" : "var(--color-accent-blue)",
                                  color: "white"
                                }}>
                                  {item.status}
                                </span>
