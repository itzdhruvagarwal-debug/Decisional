...
export default function PayoutsAdminPage() {
  ...
  const fetchWithdrawals = useCallback(async () => {
    ...
    try {
      ...
      setWithdrawals([]);
      setTotal(0);
      setError(err instanceof Error ? (err instanceof Error ? err.message : String(err)) : "Failed to load payouts");
    } finally {
      setLoading(false);
    }
  }, [filter]);
  ...
  const handleAction = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setError(err instanceof Error ? (err instanceof Error ? err.message : String(err)) : "Failed to process payout");
    } finally {
      setProcessing(null);
    }
  };
