...
export async function captureEngagement(
  ...
  try {
    ...
    let realMetrics: Partial<EngagementMetrics> = {};
    let usedRealData = false;

    // Check if YouTube
    if (deal.postUrl.includes("youtube.com") || deal.postUrl.includes("youtu.be")) {
      try {
        const ytResult = await verifyYouTubeVideoIsLive(deal.postUrl);
    ...
    } else if (deal.postUrl.includes("instagram.com")) {
      try {
        const oauth = await prisma.oAuthAccount.findFirst({
          where: { userId: deal.influencer.userId, provider: "instagram" },
          select: { accessToken: true },
        });
        const decryptedAccessToken = oauth?.accessToken ? decrypt(oauth.accessToken) : null;
        if (decryptedAccessToken) {
          const post = await findPostByUrl(decryptedAccessToken, deal.postUrl);
          if (post) {
            const likes = post.likeCount;
            const comments = post.commentsCount;
            // For Instagram image posts, we approximate views from likes (usually 6x likes)
