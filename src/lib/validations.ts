...
export const contentSubmissionSchema = z.object({
  ...
  contentUrl: z
    ...
    .refine(
      ...
      "Potentially dangerous URL context blocked",
    ),
...
}).refine(data => data.approved || (!!data.feedback && data.feedback.length > 0), {
  message: "You must provide feedback if you are rejecting the content",
  path: ["feedback"]
});
