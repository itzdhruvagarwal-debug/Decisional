...
async function getMonthlySpend(
  ...
  if (userType === "BRAND") {
    ...
    const apps = await prisma.application.findMany({
      ...
      select: { campaign: { select: { perInfluencerBudget: true } } },
    });
    return apps.reduce(
      (sum: number, a: { campaign: { perInfluencerBudget: number | null } }) =>
        sum + (a.campaign.perInfluencerBudget || 0),
      0,
    );
  }
}
...
export async function checkVerificationTierForAmount(
  ...
  try {
    ...
    throw new Error("Failed to check verification tier");
  }
}
