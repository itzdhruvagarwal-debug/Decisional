...
export async function checkPaymentFraud(
  ...
  // Rule 1: Rapid withdrawals (>= 3 in a day)
  ...
  // Rule 1.5: Daily withdrawal volume limit check (Max ₹50,000 / 5,000,000 paise in 24h)
  ...
  if (totalWithdrawn24h + params.amount > 5000000) {
    flags.push({
  ...
  // Rule 1.6: Duplicate bank account / UPI ID multi-accounting reuse check
  // Use deterministic HMAC for efficient indexed lookup instead of O(N) decryption
  if (params.bankAccount && params.bankAccount !== "UPI_PAYOUT") {
    const bankAccountHash = hashForDuplicateDetection(params.bankAccount);
    
    const duplicateBank = await prisma.withdrawal.findFirst({
      where: {
        bankAccountHash: bankAccountHash,
        wallet: { userId: { not: params.userId } },
      },
      select: { id: true },
    });
  ...
  if (params.upiId) {
    const upiIdHash = hashForDuplicateDetection(params.upiId);
  ...
  if (params.bankAccount) {
    ...
    const currentHash = hashForDuplicateDetection(params.bankAccount);
...
interface PostVerificationParams {
  ...
  dealAmount?: number; // Deal amount in paise for selective fraud checks
  engagementMetrics?: {
    followers: number;
    likes: number;
    comments: number;
...
export async function checkPostVerification(
  ...
  if (params.postUrl.includes("instagram.com") && params.influencerUserId) {
    try {
      ...
      if (decryptedAccessToken) {
        const igPost = await findPostByUrl(decryptedAccessToken, params.postUrl);
        if (igPost) {
          verifiedPostData = {
  ...
  if (!verifiedPostData && youtubeId) {
    try {
      // Try with OAuth token first if available — auto-refreshes if expired
      let accessToken: string | undefined;
      if (params.influencerUserId) {
        accessToken = (await getFreshYouTubeAccessToken(params.influencerUserId)) ?? undefined;
      }
  ...
  if (verifiedPostData.postTimestamp > params.postingDeadline) {
    ...
    riskScore += 70; // Elevated to trigger REVIEW (≥40) instead of FLAG
  }

  // Rule 6: Fake post timing check (recycled content or instant submission)
  if (params.submissionTimestamp && params.dealAcceptedAt && verifiedPostData) {
  ...
  // Rule 7: Engagement anomaly check (selective for high-value deals > ₹10,000)
  if (params.dealAmount && params.dealAmount > 1000000 && params.engagementMetrics) {
    const engagementCheck = checkEngagementAnomaly(params.engagementMetrics);
