...
export const getOrCreateHistogram = (name: string, config: any) => {
...
export const getOrCreateCounter = (name: string, config: any) => {
  let metric = client.register.getSingleMetric(name);
  if (!metric) {
...
export const getOrCreateGauge = (name: string, config: any) => {
  let metric = client.register.getSingleMetric(name);
  if (!metric) {
    metric = new client.Gauge({ name, ...config });
  }
  return metric as client.Gauge<string>;
};
