...
export async function getRecentVideos(
  ...
  try {
    ...
    return videosData.items.map((video: Record<string, unknown>) => ({
      ...
      viewCount: parseInt(((video.statistics as Record<string, string>)?.viewCount) || "0"),
      likeCount: parseInt(((video.statistics as Record<string, string>)?.likeCount) || "0"),
      commentCount: parseInt(((video.statistics as Record<string, string>)?.commentCount) || "0"),
