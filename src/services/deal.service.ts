...
export class DealService {
  private static async refundRejectPendingInvite(tx: Prisma.TransactionClient, deal: DealWithRelations) {
  ...
  private static async cancelCampaignForDirectInvite(tx: Prisma.TransactionClient, deal: DealWithRelations) {
  ...
  private static async sendAutoApproveEmails(deal: ExpiredDealCandidate) {
    try {
  ...
  private static async autoApproveDealTx(
  ...
  private static validateAndGetSubmissionUrls(deal: DealWithRelations) {
  ...
  private static async autoApproveSingleExpiredDeal(deal: ExpiredDealCandidate, now: Date): Promise<boolean> {
    const latestSubmission = deal.contentSubmissions[0];
  ...
  private static accumulateVerificationFlags(
  ...
  static async verifyPost(userId: string, dealId: string, postUrl: string): Promise<{ success: boolean; status: "VERIFIED" | "VERIFICATION_PENDING" }> {
    ...
    const { isBlocked, needsReview, verificationFlags } = DealService.accumulateVerificationFlags(
      verificationResult,
      uniquenessResult,
      privacyResult,
    );
