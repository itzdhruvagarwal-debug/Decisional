...
export class AdminAnalyticsService {
  private static cache: Record<string, { data: any; expiresAt: number }> = {};
  ...
  static async getFinancialOverview() {
    if (this.cache.financial && this.cache.financial.expiresAt > Date.now())
      return this.cache.financial.data;
    const now = new Date();
    ...
    const pendingPayouts = await prisma.paymentHold.aggregate({
      _sum: { amount: true },
      _count: true,
  ...
  static async getDashboardStats() {
    if (this.cache.dashboard && this.cache.dashboard.expiresAt > Date.now())
    ...
    const revenueToday = await prisma.transaction.aggregate({
      where: { type: "PLATFORM_FEE", createdAt: { gte: today } },
      _sum: { amount: true },
    });
