...
export function useTokenRefreshGuard() {
    ...
    useEffect(() => {
        ...
        const lastRefreshed = (session as any)?.lastRefreshed as
            | number
            | undefined;
        ...
        if (lastRefreshed !== undefined && lastRefreshed !== null) {
            ...
            if (timeUntilExpiry < 0) {
                ...
                console.warn("[SECURITY][TokenGuard] Session already expired by clock.");
                hasSignedOut.current = true;
                void signOut({
                    redirect: true,
                    callbackUrl: "/login?reason=session_expired",
    ...
    const requireFreshSession = useCallback(async (): Promise<boolean> => {
        if (!session) {
            void signOut({ redirect: true, callbackUrl: "/login?reason=no_session" });
            return false;
        }
